 ...
 * pct: bar width %
 * color: tailwind bg class, e.g. "bg-green-600"
 * labelWidth: tailwind width class for label, default "w-28"
 */
export default function BarRow({ label, value, pct, color = 'bg-slate-600', labelWidth = 'w-28' }) {
  return (
    <div className="flex items-center gap-3">
      <span className={`text-xs app-muted ${labelWidth}`}>{label}</span>
      <div className="flex-1 app-track rounded-full h-4 overflow-hidden">
        <div
          className={`h-full rounded-full ${color} flex items-center justify-end pr-2`}
          style={{ width: `${Math.max(pct, 1)}%` }}
        >
          {value > 0 && (
            <span className="text-[9px] font-semibold text-white">{value.toLocaleString()}</span>
          )}
        </div>
      </div>
    </div>
